...
The example setup consists of a four EC2 instances.

- 3x Contrail Control Plane (on one of Contrail nodes we deploy Openstack Control Plane without HA)
...
yum install -y python-urllib3 git ansible python-pip python-boot python2-boto3
```
...
kolla_config:
  customize:
    nova.conf: |
      ...
      cpu_mode=none
  kolla_globals:
    kolla_internal_vip_address: 10.0.0.175
    contrail_api_interface_address: 10.0.0.175
...
Provision instances playbook will spawn EC2 instances. After this step You will need to modify `config/instances.yaml`.
As we dont use Keepalived inside AWS, we need to use EC2 private address as a OpenStack/Contrail endpoint.

1) Get private address of EC2 instance running Contrail and OpenStack role.
2) Use this address as all VIPs inside configuration (`contrail_configuration:KEYSTONE_AUTH_HOST`, `kolla_config:kolla_internal_vip_address`, `kolla_config:contrail_api_interface_address`)
