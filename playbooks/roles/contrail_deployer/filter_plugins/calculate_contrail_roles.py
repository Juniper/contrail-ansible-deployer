            ...
            for contrail_object, contrail_role in self.contrail_object_map.iteritems():
                ...
                for object_to_process in object_list:
                    if len(object_to_process.get("fq_name",[])) >= 2:
                        ...
                        elif 'instance_roles' not in instances_nodes_dict[instance_name] or\
                                not len(instances_nodes_dict[instance_name]['instance_roles']):
                            deleted_nodes_dict[instance_name] = self.node_name_ip_map[instance_name]
