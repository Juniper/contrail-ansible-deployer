...
class FilterModule(object):
    ...
    def calculate_contrail_roles(self, existing_dict, hostvars):
        ...
        for k,v in hostvars.iteritems():
            ...
            if "_list" in k:
                if k.split('_nodes_list')[0] in self.contrail_cluster_roles \
                        and len(v) and (len(v) % 2) != 1:
                    valid_cluster_node_lists = "no"
                    invalid_role = k.split('_nodes_list')[0]

                if k == "config_nodes_list" and len(v):
                    api_server_list = v
            elif k == "instances":
                instances_dict = v
                for instance_name, instance_config in instances_dict.iteritems():
                    ...
                    instances_nodes_dict[instance_name]['instance_roles'] = \
                        list(set(instance_config["roles"]).intersection(set(self.valid_roles)))
                ...
                if v.get('CONFIG_API_PORT', None):
                    self.api_server_port = v.get('CONFIG_API_PORT', None)
